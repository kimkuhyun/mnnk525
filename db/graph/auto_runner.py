...
# ── prep ───────────────────────────────────────────────────
def prep(corp_code: str, wave: int, bsize: int):
    done = ledger_ids()
    ...
    cur.execute(
        ...
        f"WHERE corp_code=%s AND {SKIP_WHERE.replace('%', '%%')} ORDER BY chunk_id",
        (corp_code,))
    ...
    print(f"corp={corp_code}({subject}) 남은 pending={len(rows)} | 이번 배치 {nb}개(각 ≤{bsize}청크)")
...
def _load_edge(driver, conn, subject, pred, obj, ent_label, chunk_id, rcept_no):
    ...
    if pred == "hasObject":
        ...
        # SUPPLIES_TO: 공급사→주체 / RELATED_PARTY: 주체→상대
        a_name, b_name = (obj, subject) if pred == "SUPPLIES_TO" else (subject, obj)
...
def load(corp_code: str):
    ...
    for f in AUTO.glob(f"batch_{corp_code}_*.json"): f.unlink()
    for f in AUTO.glob(f"result_{corp_code}_*.json"): f.unlink()
    print(f"load 완료: 청크 {n_chunk}, 엔티티 {n_ent}, 엣지 {n_edge} (원장 기록·파일정리 완료)")
